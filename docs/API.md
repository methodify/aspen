...
| `GET /api/mesh` | — | `{ in_mesh, mesh, node, identity{node,fingerprint,certified,cert_blob,version,sha,has_root}, root_public, peers[{node,url,link_up,agents,fingerprint,health{last_error,last_error_at,last_up,last_down,last_roster,version,sha}}], relay, pending }` | membership + health; when not in a mesh, `identity.enroll_blob` if enrolled |
...
| `POST /api/mesh/pending` | `{ kind: enroll\|certify\|join\|peers_add\|relay, args }` | `{ ok, proposal, apply }` | queue a mesh change — the daemon never executes it; `aspen mesh apply` (a shell) does |
