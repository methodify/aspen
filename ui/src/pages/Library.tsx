...
function agentAddr(key: string, node: string, isSelf: (n: string) => boolean): string {
...
function sessionLead(s: SessionInfo): ReactNode {
  ...
  if (s.agent && s.state === "current") {
    ...
    return (
      <>
        ...
        <span className="sess-state">current{s.agent_live ? " · running" : ""}</span>
...
function SessionRow({ session: s, v, indent }: { session: SessionInfo; v: SessionVerbs; indent?: boolean }) {
...
function SessionList({ sessions, v }: { sessions: SessionInfo[]; v: SessionVerbs }) {
...
function RepositoriesSection({
  ...
  // and again after a row verb changed what the node knows about them.
  const [sessionsGen, setSessionsGen] = useState(0);
  const reloadSessions = () => setSessionsGen((g) => g + 1);
  useEffect(() => {
    if (!selected) {
      setSessions(null);
    ...
    let disposed = false;
    if (sessionsGen === 0) setSessions(null);
    setSessionsError(null);
    ...
    api
      ...
      .then((list) => {
        if (disposed) return;
        // A transcript with no turn is nothing yet — unless a name is on
        // it (a branch waiting for its first turn), which the row says.
        setSessions(
          list.filter((s) => s.user_messages > 0 || s.agent).sort((a, b) => b.modified - a.modified),
        );
      })
      .catch((e: unknown) => {
        if (disposed) return;
  ...
  async function reviveAgent(addr: string) {
    setActionError(null);
    try {
  ...
  /** Move a name (bare, this repo) onto a transcript: it relaunches on a
   *  branch of it; where it was is kept as an earlier point. */
  async function moveNameHere(node: string, bare: string, s: SessionInfo) {
    setActionError(null);
    const key = sessions?.find((x) => x.agent && x.agent.split("@")[0] === bare)?.agent ?? bare;
  ...
  async function ignoreBranch(node: string, s: SessionInfo) {
    if (!s.adoption_id) return;
    setActionError(null);
    try {
      await api.resolveAdoption(s.adoption_id, "ignore", undefined, isSelf(node) ? undefined : node);
      reloadSessions();
    } catch (e) {
      setActionError(errText(e));
    }
  ...
  async function forgetEarlier(node: string, s: SessionInfo) {
    if (!s.agent || !s.bookmark_id) return;
    setActionError(null);
    try {
      await api.deleteBookmark(isSelf(node) ? s.agent : `${s.agent}@${node}`, s.bookmark_id);
      reloadSessions();
    } catch (e) {
      setActionError(errText(e));
    }
                        ...
                        {sessions && sessions.length > 0 && (
                          <SessionList
                            ...
                            v={{
                              ...
                              onForget: (sess) => void forgetEarlier(n.node, sess),
                            }}
                          />
                        )}
                      </div>
